...
class ModulesController extends \kilyakus\modules\components\Controller
{
    ...
    public function actionRedirects($id)
    {
        $model = Module::findOne($id);
        ...
        if (Yii::$app->request->post('Redirects')) {
            $model->setRedirects(Yii::$app->request->post('Redirects'));
            if($model->save()){
                $this->flash('success', Yii::t('easyii', 'Module redirects updated'));
            }
            else{
                $this->flash('error', Yii::t('easyii', Yii::t('easyii', 'Update error. {0}', $model->formatErrors())));
            }
    ...
    public function actionRestoreRedirects($id)
    {
        if(($model = Module::findOne($id))){
            $model->redirects = '';
            $model->save();
            $this->flash('success', Yii::t('easyii', 'Module default redirects was restored'));
        } else {
            $this->flash('error', Yii::t('easyii', 'Not found'));
        }
        return $this->back();
    ...
    public function actionCopy($id)
    {
        ...
        if ($formModel->load(Yii::$app->request->post())){
            if(Yii::$app->request->isAjax){
                ...
                $newModulesFolder = Yii::getAlias('@bin') . '\admin' . DIRECTORY_SEPARATOR . 'modules';
                $newModuleFolder = Yii::getAlias('@bin') . '\admin' . DIRECTORY_SEPARATOR . 'modules' . DIRECTORY_SEPARATOR . $formModel->name;
                $newNameSpace = 'bin\admin\modules\\'.$formModel->name;
                ...
                $moduleFileContent = str_replace('@bin', '@bin', $moduleFileContent);
    ...
    public function actionMenuOn($id)
    {
        return $this->changeHeader($id, Module::MENU_ON);
    }

    public function actionMenuOff($id)
    {
        return $this->changeHeader($id, Module::MENU_OFF);
    }
}